...
function gradeFromFilename(filename) {
    ...
    const match =
        stem.match(/(?:^|\D)(1[0-2]|9)(?:st|nd|rd|th)?[\s_-]*grade/i) ||
        stem.match(/grade[\s_-]*(1[0-2]|9)(?!\d)/i);
    return match ? Number(match[1]) : null;
}

function localPhotoPath(source, htmlPath) {
    if (
        !source ||
    ...
    const parent = htmlPath.includes("/")
        ? htmlPath.slice(0, htmlPath.lastIndexOf("/") + 1)
        : "";
    // Only exact paths inside the saved folder are usable. Traversal and network
    // URLs are rejected, so nothing outside that folder is decoded or fetched.
    return parent + relative;
}

function localPhotoFile(source, htmlPath, filesByPath) {
    const path = localPhotoPath(source, htmlPath);
    return path ? filesByPath.get(path) || null : null;
}

function primaryRowNodes(row, selector) {
    return [...row.querySelectorAll(selector)].filter(
        (node) =>
            node.closest("tr") === row &&
            !node.closest('[id^="additional-container"]'),
    );
}
...
function parseGradeHtml(text, htmlPath, filesByPath) {
    ...
    const translated =
        /<html\b[^>]*\bclass\s*=\s*["'][^"']*\btranslated-(?:ltr|rtl)\b/i.test(
            text,
        );
    ...
    if (!table || table.tagName !== "TABLE") {
        throw new Error(
            `${filename}: no Blackbaud list-view student directory was found.`,
        );
    }
    const rows = [
    ...
    if (translated)
        warnings.push(
            `${filename} contains browser-translated text. Verify that the original grade export is the intended source.`,
        );
    for (let index = 0; index < rows.length; index += 1) {
        const row = rows[index];
        const headings = primaryRowNodes(row, "h3");
        const buttons = primaryRowNodes(
            row,
            ".user-options-button[data-userid]",
        );
        if (!headings.length && !buttons.length && row.querySelector("th"))
            continue;
        if (headings.length !== 1 || buttons.length !== 1) {
            throw new Error(
                `${filename}: student row ${index + 1} has missing or ambiguous student details.`,
            );
        }
        ...
        if (
            ...
            throw new Error(
                `${filename}: student row ${index + 1} has an invalid ID or name.`,
            );
        }
    ...
    if (!students.length)
        throw new Error(`${filename}: no student records were found.`);
    ...
    if (yearCounts.size > 1) {
        ...
        const unusual = years
            .filter((item) => item.year !== majority)
            .map((item) => item.row);
        warnings.push(
            `${filename} contains an inconsistent graduation year in student row(s) ${unusual.slice(0, 10).join(", ")}${unusual.length > 10 ? " and others" : ""}. Grades follow the export filenames.`,
        );
    }
...
async function normalizedPortrait(file) {
    if (!file || file.size > MAX_PHOTO_BYTES || !(await rasterSignature(file)))
        return null;
    let decoded;
...
async function fetchServed(baseUrl, path) {
    const url = new URL(path.split("/").map(encodeURIComponent).join("/"), baseUrl);
    try {
...
function servedFile(blob, path) {
    const file = new File([blob], path.split("/").at(-1), { type: blob.type });
    // File.webkitRelativePath is read-only, so served files carry their own path.
    Object.defineProperty(file, "relativePath", { value: path });
    return file;
}

/**
 * Fetch grade exports (for example 11thgrade.html) served at baseUrl.
 * Resolves null when the host serves none, as on GitHub Pages.
 */
export async function findServedDirectory(baseUrl, grades) {
    const found = await Promise.all(
        grades.map(async (grade) => {
            const path = `${grade}thgrade.html`;
            const blob = await fetchServed(baseUrl, path);
            return blob && servedFile(blob, path);
        }),
    );
    const files = found.filter(Boolean);
    return files.length ? { baseUrl, files } : null;
}

/**
 * Fetch the portraits a served directory references, then load everything
 * exactly like a chosen folder. onProgress also receives phase 'downloading'.
 */
export async function loadServedDirectory({ baseUrl, files }, { onProgress } = {}) {
    const paths = new Set();
    for (const file of files) {
        if (file.size > MAX_HTML_BYTES) continue;
        const fragment = document.createElement("template");
        fragment.innerHTML = await file.text();
        for (const image of fragment.content.querySelectorAll(
            "#directory-items-container img.bb-avatar-image",
        )) {
            const path = localPhotoPath(image.getAttribute("src"), file.relativePath);
    ...
    async function worker() {
        while (next < queue.length) {
            ...
            const blob = await fetchServed(baseUrl, path);
            if (blob) photos.push(servedFile(blob, path));
            completed += 1;
            if (typeof onProgress === "function")
                onProgress({ phase: "downloading", completed, total: queue.length });
        }
    }
    await Promise.all(Array.from({ length: Math.min(8, queue.length) }, worker));
    return loadDirectory([...files, ...photos], { onProgress });
}
...
export async function loadDirectory(files, { onProgress } = {}) {
    ...
    if (!selection.length)
        throw new Error(
            "Choose the folder containing your saved grade directories and their image folders.",
        );
    if (selection.length > MAX_FILES)
        throw new Error(
            `Choose a school directory folder with no more than ${MAX_FILES.toLocaleString()} files.`,
        );
    const urls = new Set();
    const dispose = () => {
    ...
    try {
        ...
        for (const file of selection) {
            if (!(file instanceof Blob) || typeof file.name !== "string")
                throw new Error("The selection contains an unsupported file.");
            const path = safeFilePath(
            ...
            if (filesByPath.has(path))
                throw new Error(
                    "The selection contains duplicate file paths. Choose one saved directory folder.",
                );
            filesByPath.set(path, file);
        }
        const htmlFiles = [...filesByPath]
            .filter(
                ([path]) =>
                    /\.html?$/i.test(path) &&
                    gradeFromFilename(path.split("/").at(-1)) !== null,
            )
            .sort(([a], [b]) => a.localeCompare(b));
        if (!htmlFiles.length)
            throw new Error(
                "No grade exports were found. Keep filenames such as 9thgrade.html or grade-11.html and include their saved image folders.",
            );
        if (htmlFiles.length > MAX_HTML_FILES)
            throw new Error(
                "Too many grade exports were selected. Choose a folder containing one current directory per grade.",
            );
        ...
        for (let index = 0; index < htmlFiles.length; index += 1) {
            ...
            if (file.size > MAX_HTML_BYTES)
                throw new Error(
                    "Each grade HTML export must be 10 MB or smaller.",
                );
            ...
            for (const record of parsed.students) {
                ...
                if (previous) {
                    if (
                        previous.grade !== record.grade ||
                        previous.name.toLowerCase() !==
                            record.name.toLowerCase()
                    ) {
                        throw new Error(
                            "The same student ID appears with conflicting names or grades. Check the selected exports.",
                        );
                    }
                    duplicateCount += 1;
                    if (!previous.photoFile)
                        previous.photoFile = record.photoFile;
                } else records.set(record.id, record);
                if (records.size > MAX_STUDENTS)
                    throw new Error(
                        `At most ${MAX_STUDENTS.toLocaleString()} students can be loaded at once.`,
                    );
            }
            notify("reading", index + 1, htmlFiles.length);
        }
        const students = [...records.values()].sort(
            (a, b) => a.grade - b.grade || a.id.localeCompare(b.id),
        );
        let next = 0;
        let completed = 0;
        ...
        const outcomes = await Promise.allSettled(
            Array.from({ length: Math.min(4, students.length) }, worker),
        );
        ...
        if (duplicateCount)
            warnings.push(
                `Merged ${duplicateCount} repeated student records with matching IDs, names, and grades.`,
            );
        const missing = students.filter((student) => !student.photoUrl).length;
        if (missing)
            warnings.push(
                `${missing} student records have no usable local portrait; those seats can keep their colored markers.`,
            );
