...
Submitted lists = the true friends, filtered so that they obey the
...
def apply_rules(lst: list[int], me: int, net: Network, rng=None, enforce_min4: bool = True,
    ...
    * >= 2 same-grade names: the policy requires two eligible choices in
      same-grade rotations. This rule alone does not ensure global feasibility.
      A list failing it is treated the same way.
    """
...
def coalition_lists(net: Network, mode: str, base: list[list[int]] | None = None) -> list[list[int]]:
    ...
    'min4'          the strongest 4-of-5 wiring found (omission star): member
                    ...
                    (minimum attained by two triples); 4+2 and all-six seatings
                    are also feasible. It passes the screen (the six-set splits 3|3).
