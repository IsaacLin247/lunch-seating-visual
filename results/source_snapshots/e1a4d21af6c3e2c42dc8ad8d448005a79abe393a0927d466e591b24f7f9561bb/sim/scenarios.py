...
def _screen_report(lists, net: Network, caps_by_state, flagged_expected: bool, coalition=None, budget=None):
    _, grade_caps = _caps_by_state(net)
    r = run_screens(lists, list(int(g) for g in net.grade), caps_by_state,
                    caps_by_grade_by_state={"same": grade_caps}, budget=budget)
...
def run_year(net: Network, lists, scenario: str, seed: int = 7, first_state: str = "mixed",
    ...
    if attendance is not None:
        extra_inputs["attendance"] = {str(r): sorted(ids[i] for i in (attendance(r) if callable(attendance)
                                                                         else attendance.get(r, attendance.get(str(r), []))))
    ...
    for state in STATES:
        ...
        p0 = Problem(grade, lists, caps, tg, hist_m, state, history_alpha=hist_a, objective=obj, staff=staff)
        key = hard_constraint_key(grade, lists, caps, tg, state, staff=staff, targets=p0.targets)
        ...
        if source_report is not None or suspected_groups:
            ...
            fm = full_model_screen(p0, per_state, budget=budget, seed=seed, deterministic=deterministic,
                                   base_status=status, groups=suspected_groups)
            full_model[state] = _full_model_ids(fm, ids)
    ...
    for r in range(rotations):
        ...
        caps, tg = table_layout(state, net.n11, net.n12)
        if attendance is not None:
            absent = list(attendance(r) if callable(attendance) else attendance.get(r, attendance.get(str(r), [])))
        else:
            absent = sample_absences(n, absence_rate, seed, r)
        ...
        if roster.conflicts:
            if conflict_policy == "waive":
                ...
                described = roster.describe(ids)["conflicts"]
                raise SchedulingConflict(
                    f"{scenario} rotation {r + 1}: {len(described)} present student(s) have no present eligible listed "
                    f"companion; an administrator must resolve this before seating", described)
        ...
        key = hard_constraint_key(grade, [[] if i in rot_waivers else lists[i] for i in range(n)], caps, tg, state,
                                  present=roster.present, staff=staff, targets=roster.targets)
