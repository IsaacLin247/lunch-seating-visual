...
from sim.constraints import SchedulingConflict  # noqa: E402
...
from sim.scenarios import (SCENARIOS, METHODS, CONFLICT_POLICIES, SubmissionReviewRequired, make_all,  # noqa: E402
                           run_config_dict, validate_trace)
from sim.solver import CPSAT_PAIR_MODES, CPSAT_EXTRA_MODES, InfeasibleInputError, SolveFailed  # noqa: E402
from sim.submissions import POLICIES, LEGACY_POLICIES  # noqa: E402
...
def summary_of(trace):
    ...
    out = {
        ...
        "extraWeight": cfg.get("objective", {}).get("extraWeight", 1.0),
        "absenceRate": cfg.get("attendance", {}).get("absenceRate", 0.0),
        "shortListPolicy": cfg.get("submissions", {}).get("policy"),
        "submissionStates": cfg.get("submissions", {}).get("counts"),
        "outcomes": {k: {kk: vv for kk, vv in v.items() if kk != "perStudent"} for k, v in trace.get("outcomes", {}).items()
...
def experiment_configuration(seed=7, scenario="honest", *, solver_seed=None,
                             ...
                             cross_grade_group_frac=0.0, short_list_policy="review",
                             nonsubmit_frac=0.0, list_cap=None, method="hybrid", extra_weight=1.0,
                             cpsat_pairs="all", cpsat_extras="count", absence_rate=0.0, conflict_policy="stop",
                             K=None):
    ...
    gen = dict(generator.arguments)
    if K is not None:
        gen["K"] = K
    if short_list_policy not in POLICIES and short_list_policy not in LEGACY_POLICIES:
        raise ValueError(f"unknown short-list policy {short_list_policy!r}")
    if method not in METHODS or conflict_policy not in CONFLICT_POLICIES:
        raise ValueError("unknown method or conflict policy")
    if cpsat_pairs not in CPSAT_PAIR_MODES or cpsat_extras not in CPSAT_EXTRA_MODES:
    ...
    return {"populationSeed": seed, "solverSeed": seed if solver_seed is None else solver_seed,
            "scenario": scenario, "generator": gen,
            "submission": {"shortListPolicy": short_list_policy, "nonsubmitFrac": nonsubmit_frac, "listCap": list_cap},
            "run": {"anneal_iters": anneal_iters, "cpsat_time": cpsat_time, "workers": workers,
                    ...
                    "rotations": rotations, "first_state": first_state, "method": method,
                    "extra_weight": extra_weight, "cpsat_pairs": cpsat_pairs, "cpsat_extras": cpsat_extras,
                    "absence_rate": absence_rate, "conflict_policy": conflict_policy}}


def expected_run_config(config):
    """The runConfig a trace produced from ``config`` must carry."""
    return run_config_dict(config["scenario"], config["solverSeed"], **config["run"])


def expected_generator_config(config):
    sub = config["submission"]
    return {**config["generator"], "short_list_policy": sub["shortListPolicy"],
            "nonsubmit_frac": sub.get("nonsubmitFrac", 0.0), "list_cap": sub.get("listCap")}
...
def _run_configuration(config, runner):
    ...
    sub = config["submission"]
    run = dict(config["run"])
    extra_weight = run.pop("extra_weight", 1.0)
    return runner(seed=config["populationSeed"], solver_seed=config["solverSeed"],
                  ...
                  short_list_policy=sub["shortListPolicy"], nonsubmit_frac=sub.get("nonsubmitFrac", 0.0),
                  list_cap=sub.get("listCap"), objective={"extraWeight": extra_weight},
                  **run)[config["scenario"]]
...
def _validate_requested_configuration(trace, config, source):
    ...
    expected_run = expected_run_config(config)
    expected_generator = expected_generator_config(config)
    checks = [
        ...
        (inputs.get("staffConstraints"), config.get("staff"), "staff constraints"),
        (inputs.get("attendance"), config.get("attendance"), "explicit attendance"),
    ...
    with manifest_lock(output):
        ...
        for category, config in jobs:
            ...
            try:
                ...
                status = ("review_required" if isinstance(exc, (SubmissionReviewRequired, SchedulingConflict)) else
                          "infeasible_input" if isinstance(exc, InfeasibleInputError) else
                          "unknown" if isinstance(exc, (TimeoutError, SolveFailed)) else "error")
                ...
                if isinstance(exc, SchedulingConflict):
                    attempt["error"]["conflicts"] = exc.conflicts[:20]
                if isinstance(exc, SubmissionReviewRequired):
                    review = exc.review or {}
                    attempt["error"]["review"] = {k: review[k] for k in ("returnedStudents", "unresolvedCandidates",
                                                                        "min4Violations", "sameGradeViolations",
                                                                        "submissions") if k in review}
...
def main(argv=None):
    ...
    ap.add_argument("--cpsat-pairs", choices=CPSAT_PAIR_MODES, default="all")
    ap.add_argument("--cpsat-extras", choices=CPSAT_EXTRA_MODES, default="count")
    ap.add_argument("--short-list-policy", default="review", choices=sorted(set(POLICIES) | set(LEGACY_POLICIES)))
    ap.add_argument("--nonsubmit-frac", type=float, default=0.0)
    ap.add_argument("--list-cap", type=int, help="truncate generated true-friend lists to this many names")
    ap.add_argument("--absence-rate", type=float, default=0.0)
    ap.add_argument("--conflict-policy", choices=CONFLICT_POLICIES, default="stop")
    ...
    base = {"solver_seed": args.solver_seed, "anneal_iters": args.anneal_iters,
            ...
            "method": args.method, "extra_weight": args.extra_weight, "cpsat_pairs": args.cpsat_pairs,
            "cpsat_extras": args.cpsat_extras, "short_list_policy": args.short_list_policy,
            "nonsubmit_frac": args.nonsubmit_frac, "list_cap": args.list_cap,
            "absence_rate": args.absence_rate, "conflict_policy": args.conflict_policy}
    jobs = []
