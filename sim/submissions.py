...
every decision is recorded per student in the trace:
  ...
  approve    pending requests within the public cap become approved exceptions;
             over-cap requests remain pending until resubmission or a cap change
...
def build_submissions(raw_lists, grade, rules: SubmissionRules, policy="review", decisions=None):
    ...
    for sub in out:
        if sub.state != "pending_review":
            continue
        if policy == "approve" and (rules.max_list is None or len(sub.names) <= rules.max_list):
...
def apply_decisions(submissions, grade, rules: SubmissionRules, decisions):
    ...
    for student, decision in decisions.items():
        ...
        if kind == "approve":
            if rules.max_list is not None and len(sub.names) > rules.max_list:
                raise ValueError(f"student {student}'s list exceeds the public cap; resubmit or change the cap before approval")
