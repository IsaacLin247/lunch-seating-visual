...
def occupancy_targets(caps, table_grade, grade, present, explicit=None, min_occupancy=2):
    ...
    """
    caps = list(caps)
    table_grade = list(table_grade)
    present = list(present)
    T = len(caps)
    if len(table_grade) != T:
        raise ValueError("each table needs an eligibility-pool label")
    if any(not isinstance(c, Integral) or c < 0 for c in caps):
        raise ValueError("physical capacities must be nonnegative integers")
    if not isinstance(min_occupancy, Integral) or min_occupancy < 0:
        raise ValueError("minimum occupancy must be a nonnegative integer")
    if len(set(present)) != len(present) or any(not isinstance(i, Integral) or not 0 <= i < len(grade) for i in present):
        raise ValueError("the present roster must contain distinct valid student indices")
    restricted = any(g is not None for g in table_grade)
    if restricted and any(g is None for g in table_grade):
        raise ValueError("automatic occupancy pools must be all unrestricted or all grade-specific")
    # Include pools with nobody present: their tables must close as well.
    pools = dict.fromkeys(table_grade, 0)
    for i in present:
        g = grade[i] if restricted else None
        pools[g] = pools.get(g, 0) + 1
    if explicit is not None:
        explicit = list(explicit)
        if len(explicit) != T or any(not isinstance(x, Integral) or not 0 <= x <= c for x, c in zip(explicit, caps)):
