...
SCENARIOS = {
    ...
    "coalition_min4": {
        ...
        "short": "Min-4 rule",
        "description": "The six must list at least four names; they use the strongest 4-of-5 wiring found (nobody lists member one), which forces at least two triples in every seating. Screens off.",
        "coalitionMode": "min4",
    ...
    "coalition_screened": {
        ...
        "short": "Screens on",
        "description": "The same-grade attack is screened on every rotation state's effective lists, flagged, and returned. The group resubmits the strongest wiring that passes (the 4-of-5 star) and the year is played with that.",
...
def build_lists(net: Network, scenario: str, short_list_policy: str = "none"):
    ...
    if mode is None:
        ...
        # adversarial resubmission: the strongest wiring that passes both screens
...
def _screen_report(lists, net: Network, caps_by_state, flagged_expected: bool):
    ...
    for state, ps in r["perState"].items():
        per_state[state] = {
            "candidates": [{"members": _ids(c["members"]), "size": c["size"], "flagged": c["flagged"],
                            **({"split": [_ids(part) for part in c["split"]]} if "split" in c else {}),
                            **({"reason": c["reason"]} if "reason" in c else {})} for c in ps["candidates"]],
            "flags": [{"screen": f["screen"], "members": _ids(f["members"]), "size": f["size"], "exact": f["exact"]}
                      for f in ps["flags"]],
            "demandFlags": [{"core": _ids(f["core"]), "followers": _ids(f["followers"]), "demand": f["demand"],
                             "seats": f["seats"], "maxTables": f["maxTables"]} for f in ps["demandFlags"]],
    ...
    return {
        ...
        "coalitionFlagged": (bool(set(r["flaggedStudents"]) & set(net.clique)) and set(net.clique) <= returned)
        if flagged_expected else None,
        "honestFlagged": _ids(sorted(m for m in returned if m not in net.clique)),
    }
...
def _versions():
    ...
    try:
        import ortools
        out["ortools"] = ortools.__version__
    except Exception:  # pragma: no cover
        pass
    try:
        out["gitCommit"] = subprocess.run(["git", "rev-parse", "--short", "HEAD"], capture_output=True, text=True,
                                          timeout=5, cwd=__file__.rsplit("/", 2)[0]).stdout.strip() or None
    except Exception:  # pragma: no cover
        out["gitCommit"] = None
    return out
...
def run_year(net: Network, lists, scenario: str, seed: int = 7, first_state: str = "mixed",
             ...
             deterministic: bool = True, feasibility_time: float = 20.0, log=print) -> dict:
    spec = SCENARIOS[scenario]
    n = net.n
...
def make_all(seed: int = 7, scenarios=None, log=print, mu: float = 0.0, omega: float = 0.0,
             cross_grade_group_frac: float = 0.0, short_list_policy: str = "none", **solver_kw) -> dict[str, dict]:
