...
def anneal(st: State, rng: random.Random, iters: int = 300_000, t0: float | None = None, t1: float | None = None):
    ...
    n = p.n
    if n == 0 or iters <= 0:
        return list(st.assign), st.cost, st.viol
...
def solve_rotation(p: Problem, seed: int = 0, anneal_iters: int = 300_000, cpsat_time: float = 3.5,
    ...
    if anneal_iters > 0:
        ...
        construction_feasible = validate_chart(p, best_assign)["valid"]
        if not construction_feasible and incumbent is not None:
    ...
    final_valid = validate_chart(p, final)["valid"]
    info["accepted"] = "anneal" if method == "hybrid" else "repair"
    if incumbent is not None and (not final_valid or incumbent_key < final_key):
    ...
    if cpsat_time > 0:
        ...
        if cp_assign is not None:
            ...
            if cb["hard"] == 0 and validate_chart(p, cp_assign)["valid"] and (not final_valid or cb["total"] <= final_cost["total"]):
    ...
    t5 = t4
    if not final_valid and method == "hybrid":
    ...
    if not final_valid and incumbent is not None:
    ...
    if incumbent is not None and list(final) == list(incumbent):
        search_feasible = construction_feasible or info["accepted"] in ("cpsat", "cpsat-fallback")
        info["status"] = "incumbent" if search_feasible and info["accepted"] != "fallback" else "fallback"
    else:
