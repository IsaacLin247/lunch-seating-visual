...
Run `python serve_local.py` and open `http://localhost:8000`. The script serves `docs/` and exposes the saved `student_directory` folder (default `~/Downloads/directory/student_directory`, or `--directory PATH`) at `/student_directory/`, on localhost only. On startup the page finds those grade exports and loads names and photos with no folder picker. The folder is never copied into `docs/`, so GitHub Pages and `python -m http.server --directory docs` serve no directory; there, click **Load directory** and select the `student_directory` folder instead. Either way the files are read in the browser and sent to no other server.
...
- **Remove directory** is available from every tab. It clears all names and portrait views, including hidden tabs, and revokes their image URLs. Refreshing or leaving the page clears a chosen folder; a served directory loads again on the next visit.
